...
class NormalSqlUtils{
  factory NormalSqlUtils()=>_getInstance();
  static NormalSqlUtils get instance => _getInstance();
  static NormalSqlUtils? _instance;
  static NormalSqlUtils _getInstance(){
    _instance??=NormalSqlUtils._internal();
    return _instance!;
  }

  NormalSqlUtils._internal();

  Future<List<PlayInfoBean>> queryPlayList()async{
    var db = await SmSqlUtils.instance.openDB();
    var list = await db.query(SmSqlTable.playInfoNormal);
    if(list.isEmpty){
      var defaultList=[
      ...
      for (var value in defaultList) {
        db.insert(SmSqlTable.playInfoNormal, value.toJson());
      }
      return defaultList;
    }
  ...
  Future<int> updatePlayedNumInfo(PlayType playType)async{
    var db = await SmSqlUtils.instance.openDB();
    var list = await db.query(SmSqlTable.playInfoNormal,where: '"type" = ? ', whereArgs: [playType.name]);
    if(list.isEmpty){
      return 0;
    }
    var map = list.first;
    ...
    await db.update(SmSqlTable.playInfoNormal, newMap,where: '"id" = ?',whereArgs: [id]);
    ...
    var queryPlayList = await NormalSqlUtils.instance.queryPlayList();
    var allPlayedNum=0;
  ...
  Future<void> unlockNextPlay(String nextPlay)async{
    var db = await SmSqlUtils.instance.openDB();
    var list = await db.query(SmSqlTable.playInfoNormal,where: '"type" = ? ', whereArgs: [nextPlay]);
    if(list.isEmpty){
      return;
    }
    var map = list.first;
  ...
  Future<void> resetPlayTime(String playType)async{
    var db = await SmSqlUtils.instance.openDB();
    var list = await db.query(SmSqlTable.playInfoNormal,where: '"type" = ? ', whereArgs: [playType]);
    if(list.isEmpty){
      return;
    }
    var map = list.first;
