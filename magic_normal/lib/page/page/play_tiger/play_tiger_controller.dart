...
import 'package:magic_normal/page/page/play_tiger/prize_bean.dart';
import 'package:magic_normal/page/page/play_tiger/tiger_bean.dart';
...
class PlayTigerController extends SmBaseController{
  ...
  _initYourList(){
    yourList.clear();
    var tigerNum = NormalValueHep.instance.getTigerNum();
    win=tigerNum!=0;
    if(tigerNum==0){
      var length = Random().nextInt(3);
      while(yourList.length<length){
        yourList.add(TigerBean(icon: "tiger7", reward: NormalValueHep.instance.getTigerReward()));
      }
    }else{
